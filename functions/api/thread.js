export async function onRequestGet(context) {
    ...
    try {
        // 1. Get Thread Title AND Author
        const thread = await env.DB.prepare("SELECT title, author_username FROM threads WHERE id = ?")
            .bind(threadId).first();
        ...
        return new Response(JSON.stringify({ 
            title: thread.title, 
            author_username: thread.author_username, // ADD THIS LINE
